...
function main()
	t1, theta1, omega1 = euler(theta0, omega0, dt, 60., 0.5)
	t2, theta2, omega2 = euler(theta0, omega0, dt, 250., 1.2)

	# plot(theta1, omega1, ".", markersize = 2)
	# xlabel("\$\\theta\$ (radians)")
	# ylabel("\$\\omega\$ (radians/s)")
	# xlim([-1,1])
	# ylim([-1,1])
	# annotate("\$\\omega\$ versus \$\\theta\$ \t \$F_D = 0.5\$", xy = (-0.5, 0.75), fontsize = 14)
	# tick_params(direction = "in", right = true, top = true)
	# xticks([-1, -0.5, 0, 0.5, 1])
	# yticks([-1, -0.5, 0, 0.5, 1])

	# savefig("Figure3.8left.pdf", bbox_inches = "tight")

	figure()
	plot(theta2, omega2, ".", markersize = 2)
	xlabel("\$\\theta\$ (radians)")
	ylabel("\$\\omega\$ (radians/s)")
	xlim([-4,4])
	ylim([-3,3])
	annotate("\$\\omega\$ versus \$\\theta\$ \t \$F_D = 1.2\$", xy = (-2, 2.5), fontsize = 14)
	tick_params(direction = "in", right = true, top = true)
	xticks([-4, -2, 0, 2, 4])
	yticks([-3, -2, -1, 0, 1, 2, 3])
	savefig("Figure3.8right.pdf", bbox_inches = "tight")
